...
const projects = [
  { number: '01', slug: 'modern-residential-development', category: 'RESIDENTIAL CONSTRUCTION', title: 'Modern Residential Development', image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1500&q=88', location: 'Lekki Phase 1, Lagos', client: 'Meridian Heights Developments', scope: 'Planning · Coordination · Construction', status: 'Completed' },
  { number: '02', slug: 'contemporary-commercial-facility', category: 'COMMERCIAL CONSTRUCTION', title: 'Contemporary Commercial Facility', image: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1500&q=88', location: 'Victoria Island, Lagos', client: 'Harbour Point Properties', scope: 'Project Management · Site Delivery · Handover', status: 'Completed' },
  { number: '03', slug: 'structured-site-execution', category: 'ENGINEERING & SITE DELIVERY', title: 'Structured Site Execution', image: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&w=1500&q=88', location: 'Ibeju-Lekki, Lagos', client: 'Northshore Industrial Projects', scope: 'Planning · Supervision · Progress Control', status: 'Delivered' },
  { number: '04', slug: 'project-coordination-delivery', category: 'PROJECT MANAGEMENT', title: 'Project Coordination & Delivery', image: 'https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=1500&q=88', location: 'Abuja, FCT', client: 'Crestview Development Group', scope: 'Programme · Resources · Stakeholder Coordination', status: 'Completed' },
]
  ...
  return (
    <div className="projects-page">
      ...
      <main>
        ...
        <section className="projects-portfolio section" id="portfolio"><div className="projects-section-head"><div><span className="section-tag">02 / SELECTED WORK</span><h2>Projects, presented<br /><span>with context.</span></h2></div><p>Click any project to open a full case study covering the client, location, scope, challenge, Segol’s role, photography, outcome and client review.</p></div>
          <div className="projects-grid">{projects.map((project) => <Link className="portfolio-project" key={project.number} to={`/projects/${project.slug}`} aria-label={`Open case study: ${project.title}`}><div className="portfolio-project-image"><img src={project.image} alt={`${project.category} project showcase`} loading="lazy" /><span>{project.number}</span><span className="portfolio-project-open">VIEW CASE STUDY <ArrowRight size={15} /></span></div><div className="portfolio-project-body"><div className="portfolio-project-top"><small>{project.category}</small><Building2 size={18} /></div><h3>{project.title}</h3><div className="portfolio-project-meta"><span><MapPin size={15} />{project.location}</span><span>{project.scope}</span></div><div className="portfolio-project-note"><CheckCircle2 size={16} /> {project.client} · {project.status}</div></div></Link>)}</div>
        </section>
