...
export default function ServiceDetail({ type }) {
  ...
  return <InnerPage eyebrow={item.eyebrow} title={item.title} accent={item.accent} intro={item.intro} dark>
    ...
    <section className="inner-section detail-case-image"><img src={item.image} alt={item.imageAlt} loading="lazy"/><div><span className="section-tag">DELIVERY IN PRACTICE</span><h2>Plan clearly.<br /><span>Build confidently.</span></h2><p>{isConstruction ? 'See the dedicated project portfolio for construction work, project context and the proof structure we are preparing for Segol.' : 'Use this area for completed project photography, site progress, drawings, team activity and project outcomes once the client supplies the real portfolio.'}</p>{isConstruction && <Link className="button dark-button" to="/projects">Explore project portfolio <ArrowRight size={17}/></Link>}</div></section>
